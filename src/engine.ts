...
const engineInit = Result(async (data: Partial<EngineData>) => {
  ...
  for (const module of ModuleData.initHooks) {
    ...
    if (!initRes.Ok) {
      return Err(`Module ${module.metadata.name} initialization failed.`, initRes.Error);
    }

    if (!initRes.Value) {
      return Err(`No data returned from module: ${module.metadata.name} initialization.`);
    }
...
const createEngine = async <T extends EngineData>(data: Partial<T>) => {
  ...
  for (const prop in initRes.Value) {
    const key = prop as keyof EngineData;
    // console.log(`Setting EngineData.${key}...`);
    EngineData[key] = (initRes.Value as any)[prop];
  }
...
export const EngineData: EngineData = {} as any;
...
type Lifecycle<U, V> = { metadata: LifecycleMetadata, hook: LifecycleFunc<U, V> };
...
export type RegisterLifecycleFunc = (metadata: Partial<LifecycleMetadata>) => <U,V>(func: LifecycleFunc<U, V>) => void;
